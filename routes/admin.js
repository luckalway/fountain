...
upload.on('end', function(fileInfo, req, res) {
	...
	if (req.uploadedFileType == "video") {
		var command = ffmpeg(fileInfo.path);
		command.output(path.join(fileInfo.uploadDir, 'video_preview.png'))
				.noAudio().on(
						'end',
						function() {
							log.info("Generated a preview image of video:"
									+ fileInfo.path);
						}).run();
		...
		req.session.messagePart.summary = req.session.messagePart.summary || {
			type : "ppt"
		};
...
router.get('/download/:messageId/:partNo/:fileName', function(req, res) {
	var file = path.join(baseUploadDir, req.params.messageId,
			req.params.partNo, req.params.fileName);
	res.download(file, req.params.fileName);
});

router.post('/messages/:messageId/parts/:partNo/videos', function(req, res,
		next) {
...
router.post('/messages/:messageId/parts/:partNo/audios', function(req, res,
		next) {
...
router.post('/messages/:messageId/parts/:partNo/summary-ppts', function(req,
		res, next) {
...
router.post('/messages/:messageId/parts/:partNo/summary-images', function(req,
		res, next) {
...
router.get('/messages/new', function(req, res, next) {
	...
	res.render('admin/messages/message-new', {
		messageId : req.session.message.id
	});
});
										...
										if (!err) {
											...
											body.rows
													.forEach(function(doc) {
														doc.value.createdDate = moment(
																doc.value.createdDate)
																.format(
																		'YYYY-MM-DD');
																	...
																	body.rows
																			.forEach(function(
																				...
																				docsMap[doc.value.messageId]['publishDates']
																						.push(doc.value.publishDate);
																			});

																	var docs = [];
																	for ( var key in docsMap) {
																		docsMap[key]['publishDates'] = docsMap[key]['publishDates']
																				.sort();
																		docs
																				.push(docsMap[key]);
																	}
...
router.get('/messages/:messageId/parts/:partNo', function(req, res, next) {
	...
	res.render('admin/messages/message-part-new', {
		messageId : messageId,
		partNo : partNo
	});
});
